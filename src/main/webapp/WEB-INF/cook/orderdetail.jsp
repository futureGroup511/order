 						...
 						<c:forEach var="i" 
							begin="${paCut.currentPage-3>0?paCut.currentPage-3:1 }"
 							...
 							<c:choose> 
 								...
 								<c:when test="${i>0 && i != paCut.currentPage &&i<=3}"> 
 									<li><a href="?page=${i }&OrderId=${paCut.data[0].orderId}&tableId=${paCut.data[0].tableId}&Orderid=${paCut.data[0].orderId}&i=${paCut.data[0].id}">${i}</a></li> 
 								</c:when> 
 							</c:choose>
 						</c:forEach> 
 						
 						<li><a 
 							href="?page=${paCut.nextPage}&OrderId=${paCut.data[0].orderId}&tableId=${paCut.data[0].tableId}&Orderid=${paCut.data[0].orderId}&i=${paCut.data[0].id}">下一页</a></li> 
               </ul>
    </div>
    </div>
    <div class="footer">
